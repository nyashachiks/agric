...
class Controller_Reports_Product_Bydistrictproduct extends Controller_Template
{
...
// We fetch each database result row into $row in turn
 //Debug::dump($query);
	...
	//		 exit;
	$customTitle = array ('Product Name','Farm Name','Size','Units','Address','District','Province','Phone');
...
while($row = mysqli_fetch_array($result)){ 
    ...
    $objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount, Model_Product::get_product_name($row['f_id']));//$row['f_id']); 
...
$objWriter->save('/tmp/by_districtproduct.xls');
File::download('/tmp/by_districtproduct.xls', 'by_districtproduct.xls');
