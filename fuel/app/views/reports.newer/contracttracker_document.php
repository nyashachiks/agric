...
$query = "SELECT distinct c.id as cid,c.contractor_id as crid,c.loan_amount as clamount,c.balance_brought_forward as bbforw,c.amount_paid as apaid,tsk.name as tname,ct.notes as ctnotes,ct.project_stage_task_id as pstid
...
// We fetch each database result row into $row in turn
 //Debug::dump($query);
	...
	//		 exit;
	$customTitle = array ('Contract ID','Contractor ID','Bal Borught Forward','Amount Paid','Task Name','Contract Notes','Project Task ID');
...
while($row = mysqli_fetch_array($result)){ 
    ...
    $objPHPExcel->getActiveSheet()->SetCellValue('B'.$rowCount, $row['crid']); 
	 $objPHPExcel->getActiveSheet()->SetCellValue('C'.$rowCount, $row['clamount']); 
	  ...
	  $objPHPExcel->getActiveSheet()->SetCellValue('D'.$rowCount, $row['bbforw']); 
	  
	   $objPHPExcel->getActiveSheet()->SetCellValue('E'.$rowCount, $row['apaid']); 
	   
	    $objPHPExcel->getActiveSheet()->SetCellValue('F'.$rowCount, $row['tname']); 
		
		 $objPHPExcel->getActiveSheet()->SetCellValue('G'.$rowCount, $row['ctnotes']); 
		  $objPHPExcel->getActiveSheet()->SetCellValue('H'.$rowCount, $row['pstid']); 
...
$objWriter->save('../../../contract_tracker.xlsx'); 
