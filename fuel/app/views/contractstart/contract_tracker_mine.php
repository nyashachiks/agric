			...
			<td>
				<div class="btn-toolbar">
					...
					<?php //$contractApplication=Model_Contractstart::query()->where(,$item['contractapplications_id']);?>
						<?php 
						$contractStart=Model_Contractstart::query()
						->where('contract_id',$item['contractapplications_id'])
						->order_by('id','desc')
						->get_one();
						if(!is_null($contractStart))
						{
						echo Html::anchor('contractstart/view/'.$contractStart->id,
						 '<i class="icon-eye-open"></i>Track', array('class' => 'btn btn-default btn-sm')); 
						 }
						 ?>	
