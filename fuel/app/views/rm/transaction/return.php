				...
				$sets=Custom_UserUtility::StatusPoll($poll);
					...
					if(!isset($sets['error']))
					{
						...
						{ 
							...
							$rm_transaction = Model_Rm_Transaction::find($ref);
							$rm_transaction->status=$sets['status'];
							$rm_transaction->paynow_ref=$sets['paynowreference'];
							$rm_transaction->save();
							
							//presentattion tweak
							if($show_button)
							{
								$sms_notification = 	" <p>A confirmation of the transaction has been  sent to the mobile number ".$rm_transaction->mobile."</p>";
							}
							else
							{
								$sms_notification = "";
							...
							if ($sets['status']=='Paid')
							{
								$rm_sale=$rm_transaction->rm_sale;
								$rm_sale->status='completed';
								$rm_sale->save();
								//Debug::dump($sale->productoffer->farmer);die;
								
								$buyer=$rm_sale->buyer->username;
								$quantity=$rm_sale->quantity;
								$base= $rm_sale->rawmaterial_offer->raw_material->measurement->unit;
								$prodName=$rm_sale->rawmaterial_offer->brand_name;
...
<?php endif; ?>
<a href="<?php echo Uri::create('rawmaterial_offer'); ?>" style="text-decoration: none">
	<button class="btn btn-md btn-warning btn-round"> Back to raw materials</button>
</a>
</div>
		</div>
