					...
					<?php foreach ($farmer_num as $item): ?>		
						<tr>
							...
							<div class="btn-toolbar">
								<div class="btn-group">
								<button type="button" class="btn btn-info btn-small" onclick="pickVendor('<?php echo $item->user_id; ?>','<?php echo Model_User::get_full_name($item->user_id); ?>')"> Pick
									</button>
								</div>
								</div>
...
<script>
	function pickVendor(farmer_id_code ,farmer_name_code)
	{
		document.getElementById('farmer_id').value=farmer_id_code;
		document.getElementById('farmer_number_code').value=farmer_name_code;
